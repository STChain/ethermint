...
func ethermintCmd(ctx *cli.Context) error {
    ...
    strategy.SetReceiver(ks.Accounts()[0].Address.String()) 
    
    ethApp, err := abciApp.NewEthermintApplication(backend, rpcClient, strategy)
    if err != nil {
        fmt.Println(err)
        os.Exit(1)
    }
    ethApp.SetLogger(emtUtils.EthermintLogger().With("module", "ethermint"))
    log.Info("keystore", "coinbae",ethApp.Receiver())
